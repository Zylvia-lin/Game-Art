...
export default function ImageToImagePage() {
  ...
  const paramsPanel = (
    <>
      ...
      >
        {submitting ? <><Loader2 className="h-4 w-4 animate-spin" />提交中...</> : <><Sparkles className="h-4 w-4" />开始编辑<span className="ml-1 text-xs opacity-80">≈{formatCostDisplay(estimateCost(resolution, 1, 1))}</span></>}
      </button>
    </>
  );
